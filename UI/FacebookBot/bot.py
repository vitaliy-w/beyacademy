...
def receive_message():
    ...
    # если запрос не был GET, э     то был POST-запрос и мы обрабатываем запрос пользователя
    else:
...
def verify_fb_token(token_sent):
    ...
    if token_sent == FBot.getVerifyToken():
...
def send_message(recipient_id, response):
    ...
    FBot.get().send_text_message(recipient_id, response)
    return 'Success'
...
def run(AccesToken, VerifyToken):
    FBot(Bot(AccesToken), AccesToken, VerifyToken)
    app.run()
...
class FBot:
    ...
    def __init__(self, bot: Bot, AccesToken: str, VerifyToken: str) -> None:
        self.bot = bot
        self.accesTokean = AccesToken
